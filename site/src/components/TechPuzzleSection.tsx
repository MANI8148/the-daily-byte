...
import { CheckCircle2, AlertCircle, HelpCircle, Grid, Cpu, Trophy, RotateCcw } from 'lucide-react';
...
export const TechPuzzleSection: React.FC<TechPuzzleSectionProps> = ({ issue }) => {
  ...
  const [isGeneratingAi, setIsGeneratingAi] = useState(false); // legacy guard kept for offline path
  ...
  // Crossword clues are set by the offline editorial desk — no AI generation.
  ...
  // Sudoku validation: a 4x4 grid is correct iff every row, column and 2x2
  // box contains 1-4 exactly once (rule-based — any valid fill passes, not
  // just one hard-coded solution).
  const isValidSudokuGrid = (grid: number[][]): boolean => {
    for (let r = 0; r < 4; r++) {
  ...
  const handleCheckSudoku = () => {
    const empty = sudokuGrid.some((row) => row.some((v) => v === 0));
    if (empty) {
      setSudokuStatus('🧩 Keep going — every cell needs a digit 1–4.');
      return;
    }
    if (isValidSudokuGrid(sudokuGrid)) {
      setSudokuStatus('🌟 PERFECT LOGIC! Tech Sudoku grid solved successfully!');
    } else {
      setSudokuStatus('❌ Rows, columns and 2×2 boxes must each hold 1–4 exactly once. Re-check!');
    }
  };
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
            ...
            <div className="md:col-span-5 flex flex-col items-center bg-[#F9F7F2] p-4 border border-[#1A1A1A]">
              ...
              {/* Clues are fixed editorial content — check, don't regenerate */}
              <div className="mt-3 w-full bg-[#1A1A1A]/90 text-[#F9F7F2] py-2 px-3 text-[10px] font-sans uppercase tracking-wider">
                Fresh clues drop with every pipeline edition
              </div>
            </div>
                  ...
                  row.map((val, cIdx) => {
                    ...
                    return (
                      <div
                        ...
                        className={`w-full h-full flex items-center justify-center font-mono-tech text-2xl font-black cursor-pointer select-none transition-colors border border-[#1A1A1A] ${cIdx % 2 === 0 ? ' border-l-4' : ''}${rIdx % 2 === 0 ? ' border-t-4' : ''} ${
                          isSelected
